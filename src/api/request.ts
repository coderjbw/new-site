...
const fetchApi = async (
  ...
  if (!response.ok) {
    ...
    if (chatbotMessage().messages.length > 0) {
      chatbotMessage().messages[chatbotMessage().messages.length - 1].progress =
        false;
    }
    throw errorData;
  }
...
export const queryWeather = (
  ...
  return fetchApi(
    `${requestUrl}/queryWeather?city=${data.city}`,
    "GET",
    "",
    "1234",
    "1234"
...
export const goodsDetails = (data: {
  goodsId: string;
}): Promise<ApiResponse<ServerGoodsDetails>> => {
